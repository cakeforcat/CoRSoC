...
def bin2hdf5(buf = 128, stride = 12, nsamples_per_file = 10000, plot_spect = False, setniq2read = True):
    ...
    h5_folder_fp = "./sdr_wifi_h5_full_empty/"
...
def preprocessing(buf = 128, test_size = 0.1,):
    ...
    for file in folder: 
        if not os.path.isdir(h5_folder_fp + file):
            ...
            label = [int(np.any(label))] * data.shape[0]     # Generate label for each training sample in file
            #print(label)
    ...
    print("Lenght of data",dataset.shape)
